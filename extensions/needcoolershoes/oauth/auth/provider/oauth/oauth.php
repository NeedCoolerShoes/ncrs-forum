...
namespace needcoolershoes\oauth\auth\provider\oauth;
...
class oauth extends base
{
	...
	public function login($username, $password)
	{
		...
		if ($this->is_set_code($service))
		{
			...
			try
			{
				$profile = $this->service_providers[$service_name]->perform_auth_login();
			...
			if (!$row)
			{
				...
				// The user does not yet exist, so create one
				$ncrs_user_row = array(
				...
				$user_id = user_add($ncrs_user_row, false);
